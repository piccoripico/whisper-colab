...
Wielojęzyczne notesy Google Colabdo transkrypcji Whisper.

## Cechy

- **Bezpłatne:** This notebook uses [openai/whisper](https://github.com/openai/whisper), which OpenAI open-sourced on January 17, 2023, together with Google Colab's free GPU runtime.
- **Brak stałego limitu rozmiaru pliku po stronie notebooka:** Można przetwarzać duże pliki audio i wideo.
...
- **Wiele formatów wyjściowych:** Notatnik generuje oryginalny transkrypt JSONoraz pochodne pliki napisów TXT, CSV, Markdown, Exceli SRT.
...
Katalog główny READMEjest punktem wejścia w języku angielskim, a dokumentacja/READMEzawiera wygenerowane zlokalizowane pliki README. Notatnik szablonu jest notatnikiem źródłowym i zawiera elementy zastępcze, takie jak `{{cell1.title}}`. Pliki regionalne zawierają READMEspecyficzną dla języka kopię i kopię do Colab. Wygenerowane notatniki są zaangażowane, więc łącza Colab pozostają stabilne.
...
Przepływ pracy CI odbudowuje pliki READMEi notatki z szablonu i plików ustawień regionalnych, sprawdza notatnik JSON, weryfikuje puste dane wyjściowe, sprawdza składnię Pythona w komórkach kodu, skanuje w poszukiwaniu oczywistych przypadkowych sekretów i kończy się niepowodzeniem, gdy wygenerowane pliki nie są zatwierdzone.

## Co CI może przetestować

- Generowanie READMEz plików ustawień regionalnych
- generowanie notatnika z plików szablonu i ustawień regionalnych
- notatnik JSONważność
- puste wyniki wykonania
- Składnia Pythona w komórkach kodu
- oczywiste przypadkowe tajemnice
- commit wygenerował READMEi pliki notatników pasujące do źródeł

Pełna ścieżka transkrypcji wymaga interfejsów API specyficznych dla Colab, pobrania modelu, dostępności GPUi plików multimedialnych dostarczonych przez użytkownika. Traktuj tę ścieżkę jako ręczny test dymny Colab po pomyślnym przejściu kontroli wygenerowanych plików.

## Licencja

Generator READMEz tego repozytorium, szablon notatnika, pliki ustawień regionalnych, wygenerowane notatniki i skrypty konserwacyjne są udostępniane na podstawie licencji MIT. Modele Whisperi zależności innych firm podlegają odpowiednim licencjom.
