...
Quaderns multilingües Google Colaber a la transcripció Whisper.

## Característiques

- **Lliure d'utilitzar:** This notebook uses [openai/whisper](https://github.com/openai/whisper), which OpenAIopen-sourced on January 17, 2023, together with Google Colab's free GPUruntime.
- **No hi ha límit fix de mida de fitxer al costat del portàtil:** Es poden processar fitxers d'àudio i vídeo grans.
...
- **Totalment en línia:** Downloads and processing are performed on Google Colab. No local computing resources are used.
- **Múltiples formats de sortida:** The notebook outputs the original JSONtranscript and derived TXT, CSV, Markdown, Excel, and SRTsubtitle files.

## Idiomes

| Llengua | README | Quadern |
| --- | --- | --- |
...
L'arrel READMEés el punt d'entrada en anglès, i docs/READMEconté els fitxers READMElocalitzats generats. El quadern de plantilla és el quadern d'origen i conté marcadors de posició com ara `{{cell1.title}}`. Els fitxers de configuració regional proporcionen READMEespecífica de l'idioma i una còpia orientada a Colab. Els quaderns generats estan compromesos perquè els enllaços de Colab es mantinguin estables.
...
El flux de treball CI reconstrueix els fitxers READMEi els quaderns a partir de la plantilla i els fitxers de configuració regional, verifica el quadern JSON, verifica les sortides buides, comprova la sintaxi de Python a les cel·les de codi, cerca secrets accidentals evidents i falla quan no es comprometen els fitxers generats.

## Què pot provar CI

- READMEgeneració a partir de fitxers de configuració regional
- generació de blocs de notes a partir de la plantilla i els fitxers de configuració regional
- quadern JSONvalidesa
- sortides d'execució buides
- Sintaxi de Python a les cel·les de codi
- secrets accidentals evidents
- commit generat READMEi fitxers de bloc de notes que coincideixen amb les fonts

La ruta de transcripció completa requereix API específiques de Colab, descàrregues de models, disponibilitat GPUi fitxers multimèdia proporcionats per l'usuari. Tracteu aquest camí com una prova de fum de Colab manual després que passin les comprovacions dels fitxers generats.

## llicència

El generador READMEd'aquest dipòsit, la plantilla de quadern, els fitxers de configuració regional, els quaderns generats i els scripts de manteniment es publiquen sota la llicència MIT. Els models Whisperi les dependències de tercers es regeixen per les seves respectives llicències.
