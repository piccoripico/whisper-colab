...
Köp dilli Google Colabtranskripsiýasy üçin depderler.

## Aýratynlyklary

- **Ulanmak üçin mugt:** This notebook uses [openai/whisper](https://github.com/openai/whisper), which OpenAI open-sourced on January 17, 2023, together with Google Colab's free GPU runtime.
- **Bellenen depderçäniň gapdalyndaky faýl ölçeg çäkleri ýok:** Uly ses we wideo faýllary gaýtadan işlenip bilner.
- **Topary gaýtadan işlemek:** Bir gezekde birnäçe ses we wideo faýly gaýtadan işläp bolýar.
- **Google Drivegoldaw:** Faýllary gönüden-göni ýükläp bolýar, ýa-da Google Drive-da saklanýan ses / wideo faýllaryny gaýtadan işläp bolýar.
- **Doly onlaýn:** @ Üklemeler we gaýtadan işlemek Google Colab. Localerli hasaplaýyş çeşmeleri ulanylmaýar.
- **Birnäçe çykyş formatlary:** Notebook asyl JSONtranskripsiýasyny çykarýar we alnan TXT, CSV, Markdown, Excelwe SRTsubtitr faýllaryny alýar.
...
Kök READMEiňlisçe giriş nokady, we resminamalar / READMEdöredilen lokallaşdyrylan READMEfaýllary bar. Şablon depderçesi çeşme depderçesi bolup, `{{cell1.title}}`ýaly ýer eýelerini öz içine alýar. Fileerli faýllar dile mahsus READMEwe Colab-yň göçürmesini üpjün edýär. Döredilen depderçeler, şonuň üçin Colab baglanyşyklary durnukly bolar.
...
CI iş tertibi şablondan we ýerli faýllardan READMEfaýllary we depderçeleri täzeden gurýar, depderini JSONbarlaýar, boş netijeleri barlaýar, kod öýjüklerinde Python sintaksisini barlaýar, tötänleýin syrlary gözden geçirýär we döredilen faýllar ýerine ýetirilmedik ýagdaýynda şowsuz bolýar.

## CI synag edip biljek zady

- Localerli faýllardan README nesli
- şablondan we ýerli faýllardan depder ýazmak
- depder JSONgüýji
- boş ýerine ýetiriş netijeleri
- Kod öýjüklerinde Python sintaksis
- aç-açan tötänleýin syrlar
- döredilen READMEwe çeşmelere gabat gelýän depder faýllary

Doly transkripsiýa ýoly, Colab-a mahsus API-leri, modeli göçürip almagy, GPUelýeterliligini we ulanyjy tarapyndan üpjün edilen media faýllaryny talap edýär. Döredilen faýl barlaglaryndan soň, bu ýoly el bilen Colab tüsse synagy hökmünde görüň.

## Ygtyýarnama

Bu ammaryň READMEgeneratory, depder şablony, ýerli faýllar, döredilen depderler we tehniki hyzmat ýazgylary MITYgtyýarnamasy boýunça goýberilýär. Whispermodeller we üçünji taraplara baglylyklar degişli ygtyýarnamalary bilen dolandyrylýar.
