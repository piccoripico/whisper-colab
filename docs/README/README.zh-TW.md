...
用於Whisper轉錄的多語言Google Colab筆記本。

## 特點

- **免費使用:** 筆記本使用[openai/whisper](https://github.com/openai/whisper)（OpenAI於2023年1月17日開源），以及Google Colab的免費GPU運行時。
- **沒有固定的筆記本端檔案大小限制:** 可以處理大型音訊和視訊檔案。
- **批量處理:** 一次運行可以處理多個音訊和視訊檔案。
- **Google Drive支持:** 可以直接上傳文件，也可以處理Google Drive中儲存的音訊視訊檔案。
- **全面上線:** 下載和處理在Google Colab上執行。不使用本地運算資源。
- **多種輸出格式:** 筆記本輸出原始JSON轉錄本和衍生的TXT、CSV、Markdown、Excel和SRT字幕檔。
...
根README是英文入口點，docs/README包含生成的本地化README文件。範本筆記本是來源筆記本，包含佔位符，例如 `{{cell1.title}}`。區域設定檔提供特定於語言的 README和麵向 Colab 的副本。生成的筆記本已提交，因此 Colab 連結保持穩定。
...
CI 工作流程從範本和區域設定檔重建 README檔案和筆記本，檢查筆記本 JSON，驗證空輸出，檢查程式碼單元中的 Python 語法，掃描明顯的意外秘密，並在未提交產生的檔案時失敗。
...
- 程式碼單元中的 Python 語法
- 明顯的意外秘密
- 提交產生的 README和與來源相符的筆記本文件

完整的轉錄路徑需要 Colab 特定的 API、模型下載、GPU可用性以及使用者提供的媒體檔案。產生檔案檢查通過後，將路徑視為手動 Colab 冒煙測試。

## 許可證

此儲存庫的 README產生器、筆記本範本、區域設定檔、產生的筆記本和維護腳本是根據 MIT授權發布的。 Whisper模型和第三方相依性受其各自的許可證管轄。
