...
Whisper전사를 위한 다국어 Google Colab노트북.

## 특징

- **무료로 사용 가능:** 이 노트북은 Google Colab의 무료 GPU런타임과 함께 2023년 1월 17일에 오픈 소스로 공개된 [openai/whisper](https://github.com/openai/whisper)을 사용합니다.
- **고정된 노트북 측 파일 크기 제한 없음:** 대용량 오디오 및 비디오 파일을 처리할 수 있습니다.
- **일괄 처리:** 한 번의 실행으로 여러 오디오 및 비디오 파일을 처리할 수 있습니다.
- **Google Drive지원:** 파일을 직접 업로드하거나 Google Drive에 저장된 오디오/비디오 파일을 처리할 수 있습니다.
- **완전 온라인:** 다운로드 및 처리는 Google Colab에서 수행됩니다. 로컬 컴퓨팅 리소스는 사용되지 않습니다.
- **다양한 출력 형식:** 노트북은 원본 JSON성적표와 파생된 TXT, CSV, Markdown, Excel및 SRT자막 파일을 출력합니다.
...
루트 README은 영어 진입점이고 docs/README에는 생성된 지역화된 README파일이 포함되어 있습니다. 템플릿 노트북은 소스 노트북이며 `{{cell1.title}}`과 같은 자리 표시자를 포함합니다. 로케일 파일은 언어별 README및 Colab용 사본을 제공합니다. 생성된 노트북은 커밋되므로 Colab 링크는 안정적으로 유지됩니다.
...
CI 워크플로는 템플릿 및 로케일 파일에서 README파일과 노트북을 다시 작성하고, JSON노트북을 확인하고, 빈 출력을 확인하고, 코드 셀에서 Python 구문을 확인하고, 명백한 우발적 비밀을 검색하고, 생성된 파일이 커밋되지 않으면 실패합니다.

## CI가 테스트할 수 있는 것

- 로케일 파일에서 README생성
- 템플릿 및 로케일 파일에서 노트북 생성
- 노트북 JSON유효성
- 빈 실행 출력
- 코드 셀의 Python 구문
- 명백한 우발적 비밀
- 커밋된 생성된 README및 소스와 일치하는 노트북 파일

전체 전사 경로에는 Colab 관련 API, 모델 다운로드, GPU가용성, 사용자 제공 미디어 파일이 필요합니다. 생성된 파일 검사가 통과된 후 해당 경로를 수동 Colab 스모크 테스트로 처리합니다.

## 라이센스

이 저장소의 README생성기, 노트북 템플릿, 로캘 파일, 생성된 노트북 및 유지 관리 스크립트는 MIT라이선스에 따라 릴리스됩니다. Whisper모델 및 타사 종속성은 해당 라이선스의 적용을 받습니다.
