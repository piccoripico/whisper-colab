...
Daugiakalbiai Google Colabbloknotai, skirti Whispertranskripcijai.

## Savybės

- **Nemokamai naudoti:** Šiame bloknote naudojama [openai/whisper](https://github.com/openai/whisper), kuri OpenAIbuvo atviro kodo 2023 m. sausio 17 d., kartu su Google Colabnemokama GPUvykdymo laiku.
- **Nėra fiksuoto nešiojamojo kompiuterio failo dydžio apribojimo:** Galima apdoroti didelius garso ir vaizdo failus.
- **Paketinis apdorojimas:** Vienu paleidimu galima apdoroti kelis garso ir vaizdo failus.
- **Google Drivesupport:** Failus galima įkelti tiesiogiai arba apdoroti Google Drivesaugomus garso/vaizdo failus.
- **Visiškai internete:** Downloads and processing are performed on Google Colab. Nenaudojami jokie vietiniai skaičiavimo ištekliai.
- **Keli išvesties formatai:** The notebook outputs the original JSONtranscript and derived TXT, CSV, Markdown, Excel, and SRTsubtitle files.

## Kalbos

| Kalba | README | Užrašų knygelė |
| --- | --- | --- |
...
Šaknis READMEyra anglų kalbos įvesties taškas, o docs/READMEyra sugeneruoti lokalizuoti READMEfailai. Šablonų bloknotas yra šaltinio bloknotas, kuriame yra vietos rezervavimo ženklų, pvz., `{{cell1.title}}`. Lokalės failai pateikia konkrečios kalbos READMEir „Colab“ kopiją. Sugeneruotos užrašinės yra skirtos, todėl „Colab“ nuorodos išlieka stabilios.
...
CI darbo eiga atkuria READMEfailus ir bloknotus iš šablono ir lokalės failų, tikrina bloknotą JSON, patikrina tuščias išvestis, tikrina Python sintaksę kodo langeliuose, nuskaito, ar nėra akivaizdžių atsitiktinių paslapčių, ir nepavyksta, kai sugeneruoti failai neįpareigoti.

## Ką CI gali išbandyti

- READMEgeneravimas iš lokalių failų
- nešiojamojo kompiuterio generavimas iš šablono ir lokalės failų
- bloknotas JSONgaliojimas
- tušti vykdymo išėjimai
- Python sintaksė kodo ląstelėse
- akivaizdžios atsitiktinės paslaptys
- įsipareigojo generuoti READMEir bloknoto failai, atitinkantys šaltinius

Norint pasiekti visą transkripcijos kelią, reikia specialių „Colab“ API, modelių atsisiuntimų, GPUasiekiamumo ir naudotojo pateiktų medijos failų. Laikykite šį kelią kaip neautomatinį „Colab“ dūmų testą, kai sugeneruotų failų patikros bus sėkmingos.

## Licencija

Šios saugyklos READMEgeneratorius, bloknoto šablonas, lokalės failai, sugeneruoti bloknotai ir priežiūros scenarijai išleidžiami pagal MITlicenciją. Whispermodelius ir trečiųjų šalių priklausomybes reglamentuoja atitinkamos jų licencijos.
