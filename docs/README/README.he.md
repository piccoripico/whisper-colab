...
מחברות Google Colabרב לשוניות עבור תמלול Whisper.

## תכונות

- **חינם לשימוש:** מחברת זו משתמשת ב-[openai/whisper](https://github.com/openai/whisper), ש-OpenAI קיבלה בקוד פתוח ב-17 בינואר 2023, יחד עם זמן הריצה החינמי של GPU של Google Colab.
- **אין מגבלה קבועה של גודל קובץ בצד המחברת:** ניתן לעבד קבצי אודיו ווידאו גדולים.
- **עיבוד אצווה:** ניתן לעבד קבצי אודיו ווידאו מרובים בהרצה אחת.
- **תמיכה ב-Google Drive:** ניתן להעלות קבצים ישירות, או לעבד קבצי אודיו/וידאו המאוחסנים ב-Google Drive.
- **מקוון לחלוטין:** ההורדות והעיבוד מתבצעים ב-Google Colab. לא נעשה שימוש במשאבי מחשוב מקומיים.
- **פורמטי פלט מרובים:** המחברת מפלטת את תמליל JSON המקורי וקובצי כתוביות מסוג TXT, CSV, Markdown, Excel ו-SRT.
...
השורש READMEהוא נקודת הכניסה באנגלית, ו-docs/READMEמכיל קבצי READMEמקומיים שנוצרו. מחברת התבנית היא מחברת המקור ומכילה מצייני מיקום כגון `{{cell1.title}}`. קובצי מיקום מספקים READMEספציפי לשפה ועותק הפונה ל-Colab. מחברות שנוצרו מחויבות כך שקישורי Colab נשארים יציבים.
...
זרימת העבודה של CI בונה מחדש READMEקבצים ומחברות מקבצי התבנית והמקום, בודק את המחברת JSON, מאמת פלטים ריקים, בודק תחביר Python בתאי קוד, סורק לאיתור סודות מקריים ברורים ונכשל כאשר קבצים שנוצרו אינם מחויבים.

## מה CI יכול לבדוק

- יצירת READMEמקבצי מקומיים
- יצירת מחברת מהתבנית וקובצי המקום
- תוקף המחברת JSON
- פלטי ביצוע ריקים
- תחביר פייתון בתאי קוד
- סודות מקריים ברורים
- מחויב שנוצר READMEוקבצי מחברת התואמים את המקורות

נתיב התמלול המלא דורש ממשקי API ספציפיים ל-Colab, הורדות דגמים, זמינות GPUוקובצי מדיה שסופקו על ידי המשתמש. התייחס לנתיב הזה כאל בדיקת עשן ידנית של Colab לאחר שעברו בדיקות קבצים שנוצרו.

## רישיון

מחולל READMEשל מאגר זה, תבנית מחברת, קבצי מקומיים, מחברות שנוצרו ותסריטי תחזוקה משוחררים תחת הרישיון MIT. Whisperדגמים ותלות של צד שלישי כפופים לרשיונות המתאימים שלהם.
