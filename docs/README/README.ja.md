...
Whisper文字起こし用の多言語 Google Colabノートブック。

## 特長

- **無料で使用できます:** このノートブックは、OpenAIが 2023 年 1 月 17 日にオープンソース化した [openai/whisper](https://github.com/openai/whisper)と、Google Colabの無料の GPUランタイムを使用します。
- **ノートブック側のファイル サイズの固定制限なし:** 大きなオーディオ ファイルやビデオ ファイルを処理できます。
- **バッチ処理:** 複数のオーディオ ファイルとビデオ ファイルを 1 回の実行で処理できます。
- **Google Driveサポート:** ファイルを直接アップロードすることも、Google Driveに保存されているオーディオ/ビデオ ファイルを処理することもできます。
- **完全オンライン:** ダウンロードと処理は Google Colabで実行されます。ローカルのコンピューティング リソースは使用されません。
- **複数の出力フォーマット:** ノートブックは、元の JSONトランスクリプトと、派生した TXT、CSV、Markdown、Excel、および SRT字幕ファイルを出力します。
...
ルート READMEは英語のエントリ ポイントであり、docs/READMEには生成されたローカライズされた READMEファイルが含まれています。テンプレート ノートブックはソース ノートブックであり、`{{cell1.title}}`などのプレースホルダーが含まれています。ロケール ファイルは、言語固有の READMEおよび Colab 向けのコピーを提供します。生成されたノートブックはコミットされるため、Colab リンクは安定した状態に保たれます。
...
CI ワークフローは、テンプレート ファイルとロケール ファイルから READMEファイルとノートブックを再構築し、ノートブック JSONをチェックし、空の出力を検証し、コード セル内の Python 構文をチェックし、明らかな偶発的なシークレットをスキャンし、生成されたファイルがコミットされていない場合は失敗します。

## CI でテストできること

- READMEロケール ファイルからの生成
- テンプレートとロケール ファイルからのノートブックの生成
- ノートブックJSONの有効性
- 空の実行出力
- コードセル内のPython構文
- 明らかな偶然の秘密
- 生成された READMEとソースに一致するノートブック ファイルをコミットしました

完全な転写パスには、Colab 固有の API、モデルのダウンロード、GPUの可用性、およびユーザー提供のメディア ファイルが必要です。生成されたファイルのチェックに合格した後、そのパスを手動の Colab スモーク テストとして扱います。

## ライセンス

このリポジトリの READMEジェネレーター、ノートブック テンプレート、ロケール ファイル、生成されたノートブック、およびメンテナンス スクリプトは、MITライセンスに基づいてリリースされています。 Whisperモデルとサードパーティの依存関係は、それぞれのライセンスによって管理されます。
