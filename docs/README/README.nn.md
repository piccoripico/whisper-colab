...
Flerspråklige Google Colabnotatbøker for Whispertranskripsjon.

## Funksjoner

- **Gratis å bruke:** Denne notatboken bruker [openai/whisper](https://github.com/openai/whisper), som OpenAIåpnet kildekode 17. januar 2023, sammen med Google Colabsin gratis GPUkjøretid.
- **Ingen fast filstørrelsesgrense på notatboksiden:** Store lyd- og videofiler kan behandles.
- **Batchbehandling:** Flere lyd- og videofiler kan behandles i en kjøring.
- **Google Drivestøtte:** Filer kan lastes opp direkte, eller lyd-/videofiler lagret i Google Drivekan behandles.
- **Helt på nett:** Nedlastinger og behandling utføres på Google Colab. Ingen lokale dataressurser brukes.
- **Flere utdataformater:** Den bærbare datamaskinen sender ut den originale JSON-transkripsjonen og avledede TXT, CSV, Markdown, Excelog SRTundertekstfiler.
...
Roten READMEer det engelske inngangspunktet, og docs/READMEinneholder genererte lokaliserte README-filer. Malnotatboken er kildenotatboken og inneholder plassholdere som `{{cell1.title}}`. Lokale filer gir språkspesifikk READMEog Colab-vendt kopi. Genererte notatbøker er forpliktet slik at Colab-lenker forblir stabile.
...
CI-arbeidsflyten gjenoppbygger READMEfiler og notatbøker fra malen og lokalitetsfilene, sjekker notatboken JSON, verifiserer tomme utdata, sjekker Python-syntaks i kodeceller, skanner etter åpenbare tilfeldige hemmeligheter, og mislykkes når genererte filer ikke er committert.

## Hva CI kan teste

- READMEgenerasjon fra lokalitetsfiler
- notatbokgenerering fra mal- og lokalitetsfilene
- notatbok JSONgyldighet
- tomme utførelsesutganger
- Python-syntaks i kodeceller
- åpenbare tilfeldige hemmeligheter
- committed genererte READMEog notatbokfiler som samsvarer med kildene

Den fullstendige transkripsjonsbanen krever Colab-spesifikke APIer, modellnedlastinger, GPUtilgjengelighet og brukerleverte mediefiler. Behandle denne banen som en manuell Colab-røyktest etter at kontroller av genererte filer er bestått.

## Lisens

Dette depotets README-generator, notatbokmal, lokalitetsfiler, genererte notatbøker og vedlikeholdsskript er utgitt under MIT-lisensen. Whispermodeller og tredjepartsavhengigheter styres av deres respektive lisenser.
